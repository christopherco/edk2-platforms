...
//
// The following implementation of SdhcWriteBlockData is buggy and results into
// data corruption. Keep it for reference.
//
#if 0
EFI_STATUS
SdhcWriteBlockData(
    ...
    )
{
    ...
    ASSERT(WtmkLvl.Fields.WR_WML > 0);

    while (WordIdx < NumWords) {
DEBUG_CODE_BEGIN ();
        USDHC_BLK_ATT_REG blkAtt; blkAtt.AsUint32 = MmioRead32((UINTN)&Reg->BLK_ATT);
        ASSERT (blkAtt.Fields.BLKCNT > 0);
DEBUG_CODE_END ();

        Status = WaitForWriteFifo(SdhcCtx);
        if (EFI_ERROR(Status)) {
            LOG_ERROR(
                "WaitForWriteFifo() failed at Word%d. %r",
                (UINT32)WordIdx,
                Status);
            return Status;
        }

        FifoWords = WtmkLvl.Fields.WR_WML;
        while (WordIdx < NumWords && FifoWords > 0) {
            MmioWrite32((UINTN)&Reg->DATA_BUFF_ACC_PORT, Buffer[WordIdx]);
            ++WordIdx;
            --FifoWords;
        }
    }
