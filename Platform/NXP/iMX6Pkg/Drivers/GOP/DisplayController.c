    ...
    DISPLAY_TIMING* DisplayTimingPtr)
{
    ...
    wrChConfigReg.FILED_MODE = 0; // Frame mode what is field mode?
    wrChConfigReg.CHAN_MASK_DEFAULT = 0; // only used highest priority
    wrChConfigReg.PROG_CHAN_TYP = 0; // Begin as disable
    wrChConfigReg.PROG_DISP_ID = DisplayInterface; // Verify this
    ...
    // TODO : Dig more into channel 1 implementation
    OS_ZERO_MEM((void*)&wrChConfigReg, sizeof(wrChConfigReg));
    wrChConfigReg.FILED_MODE = 0; // Frame mode what is field mode?
    wrChConfigReg.CHAN_MASK_DEFAULT = 0; // only used highest priority
    ...
    // TODO : Create utility function to determine
    // Setup bus mapping unit
    // For now use a simple mapping where Byte 0, 1, 2 for pointer 1
    // are mapped to mapping unit parameter 0, 1, 2
    {
