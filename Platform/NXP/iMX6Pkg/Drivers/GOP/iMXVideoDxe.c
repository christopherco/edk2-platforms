    ...
    )
{
    ...
    if (ModeNumber != 0) {
        DEBUG((EFI_D_ERROR, "VidGopQueryMode: Saw request to query mode %d\n", ModeNumber));
    ...
    )
{
    ...
    if (ModeNumber != 0) {
        DEBUG((EFI_D_ERROR, "VidGopSetMode: Saw request to set mode to %d\n", ModeNumber));
    ...
    )
{
    ...
    if (BltOperation == EfiBltVideoFill) {
        ...
        DEBUG((EFI_D_ERROR, "VideoDxe:VidGopBlt not impl %d\n", BltOperation));
