...
/*
  Define with 1 to enable dumping TA var log to the console and persistent media
*/
#define DUMP_VAR_LOG 0
...
/**
  ...
  @param[out]     WtrStatus          The actual WTR status result of the executed command.
  ...
  )
{
  ...
  *WtrStatus = TeecOperation.params[2].value.b;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    LOG_ERROR ("OpteeRuntimeVariableInvokeCommand(VSGetLogOp) failed. (Status=%r)");
    goto Exit;
  }

  if (WtrStatus != STATUS_BUFFER_TOO_SMALL) {
    Status = EFI_PROTOCOL_ERROR;
    LOG_ERROR ("Unexpected TA failure getting the var log size. (WtrStatus=0x%08x)", WtrStatus);
    goto Exit;
  }

  if (ResultSize == 0) {
    LOG_INFO ("Nothing to dump, the var log is empty");
    goto Exit;
  }

  LogBuffer.size  = ResultSize;

  TEEC_Result result = TEEC_AllocateSharedMemory (&mTeecContext, &LogBuffer);
  if (result != TEEC_SUCCESS) {
    Status = EFI_OUT_OF_RESOURCES;
    LOG_ERROR ("Can't allocate space from the secure memory for the var log");
    goto Exit;
  }
  ...
  if (EFI_ERROR (Status)) {
    LOG_ERROR ("OpteeRuntimeVariableInvokeCommand(VSGetLogOp) failed. (Status=%r)", Status);
    goto Exit;
  }

  if (WtrStatus != 0) {
    Status = EFI_PROTOCOL_ERROR;
    LOG_ERROR ("Unexpected TA failure reading the var log. (WtrStatus=0x%08x)", WtrStatus);
    goto Exit;
  }

  UINTN LogCount = LogBuffer.size / sizeof(VAR_LOG_ENTRY);
  LOG_INFO ("Dumping var log of %u entries:", LogCount);

  VAR_LOG_ENTRY *LogEntry = (VAR_LOG_ENTRY *) LogBuffer.buffer;

  UINTN i;
  for (i = 0; i < LogCount; i++) {
    LOG_INFO (
      "%u@%u %a(%s, %d) %ums (0x%08x)",
      (UINT32)i,
      LogEntry->StartTimeMs,
      mOperationStr[LogEntry->Operation],
      LogEntry->StrParam,
      LogEntry->IntParam,
      LogEntry->DurationMs,
      LogEntry->Status);
  ...
  Status = OpteeRuntimeVariableWriteLogToDisk (
            (VAR_LOG_ENTRY *) LogBuffer.buffer,
            LogCount);

  if (EFI_ERROR (Status)) {
    LOG_ERROR ("OpteeRuntimeVariableWriteLogToDisk() failed. (Status=%r)", Status);
    goto Exit;
  }

Exit:
  TEEC_ReleaseSharedMemory (&LogBuffer);
  return Status;
}
...
#if DUMP_VAR_LOG
  Status = OpteeRuntimeVariableDumpLog ();
  if (Status != EFI_SUCCESS) {
    LOG_ERROR ("OpteeRuntimeVariableDumpLog() failed. (Status=%r)", Status);
  }
#endif
