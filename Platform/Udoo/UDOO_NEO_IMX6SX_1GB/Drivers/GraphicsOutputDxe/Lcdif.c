    ...
    )
{
    ...
    //
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_RL_CLR, IMX6SX_LCDIF_RL_SFTRST);
    do
    {
        value32 = MmioRead32(IMX6SX_PHYSADDR_LCDIF1_RL);
    } while (value32 & IMX6SX_LCDIF_RL_SFTRST);
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_RL_CLR, IMX6SX_LCDIF_RL_CLKGATE);
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_RL_SET, IMX6SX_LCDIF_RL_SFTRST);
    do
    {
        value32 = MmioRead32(IMX6SX_PHYSADDR_LCDIF1_RL);
    ...
    )
{
    ...
    // 24-bit hard coded
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_RL,
        IMX6SX_LCDIF_RL_BYPASS_COUNT |
        IMX6SX_LCDIF_RL_DOTCLK_MODE |
        IMX6SX_LCDIF_RL_LCD_DATABUS_WIDTH_24 |
        IMX6SX_LCDIF_RL_WORD_LENGTH_24 |
        IMX6SX_LCDIF_RL_MASTER
        );

    // first three bytes out of every 32 contain valid data
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_CTRL1,
        (0x7 << IMX6SX_LCDIF_CTRL1_BYTE_PACKING_FORMAT_SHL));

    // Valid vertical and horizontal data
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_TRANSFER_COUNT,
        (Timing->VActive << IMX6SX_LCDIF_TRANSFER_COUNT_V_SHL) |
        Timing->HActive);

    // V-Sync period
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL0,
    ...
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL1, Timing->VActive + Timing->VBlank);

    // H-Sync length and period
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL2,
    ...
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL3,
    ...
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL4, Timing->HActive);

    // frame buffer location
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_CUR_BUF, (UINT32)FrameBuffer);
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_NEXT_BUF, (UINT32)FrameBuffer);

    // set clear
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_CTRL1_SET, IMX6SX_LCDIF_CTRL1_FIFO_CLEAR);

    // turn on sync signals
    value32 = MmioRead32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL4);
    value32 |= IMX6SX_LCDIF_VDCTRL4_SYNC_SIGNALS_ON;
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_VDCTRL4, value32);

    // clear FIFO clear
    MmioWrite32(IMX6SX_PHYSADDR_LCDIF1_CTRL1_CLR, IMX6SX_LCDIF_CTRL1_FIFO_CLEAR);
