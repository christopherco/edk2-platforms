    ...
    )
{
    ...
    edidPreferredTiming =
        (DETAILED_TIMING_DESCRIPTOR*)&sgInst.EdidData[EDID_DTD_1_OFFSET];

    status = ConvertDTDToDisplayTiming(edidPreferredTiming, PreferredTiming);
    ...
    //
    // Do not support odd resolution width and height. Do not support any resolution
    // that has a higher pixel clock than the default pixel clock.
    // 
    if ((PreferredTiming->HActive < 480) || (PreferredTiming->HActive < 640) ||
        ((PreferredTiming->HActive % 4) != 0) || ((PreferredTiming->VActive % 4) != 0) ||
         (PreferredTiming->PixelClock > DefaultTiming.PixelClock)) {
