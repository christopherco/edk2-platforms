/** @file
*  platform initialization for iMX6 Solo SOM
*
...
static VOID GateUnusedClocks()
{
  ...
  DEBUG ((EFI_D_INIT, "++GateUnusedClocks() S\r\n"));
...
/**
  Initialize clock and power for modules on the iMX6 Solo SOM.
  Gate unused clocks is not called since it is Quad-Dual specific and not applcable to SDL
**/
VOID ImxClkPwrInit ()
{
  EFI_STATUS status;
  DEBUG ((EFI_D_INIT, "++ImxClkPwrInit S. StatusPtr=%p\r\n", (UINTN*)&status));

  //GateUnusedClocks();
  //DEBUG ((EFI_D_INIT, "ImxClkPwrInit S gated unused clocks\r\n"));
  //UngateRequiredClocks();
  //DEBUG ((EFI_D_INIT, "ImxClkPwrInit S ungated required clocks\r\n"));

  if (FeaturePcdGet (PcdGpuEnable))
  {
    DEBUG ((EFI_D_INIT, "ImxClkPwrInit S will call ImxClkPwrGpuEnable()\r\n"));
    status = ImxClkPwrGpuEnable ();
    ASSERT_EFI_ERROR (status);
  }
  DEBUG ((EFI_D_INIT, "ImxClkPwrInit S will call ImxClkPwrIpuDIxEnable()\r\n"));
  status = ImxClkPwrIpuDIxEnable();
  ASSERT_EFI_ERROR (status);
  DEBUG ((EFI_D_INIT, "ImxClkPwrInit S will call ImxClkPwrValidateClocks()\r\n"));
  status = ImxClkPwrValidateClocks();
  ASSERT_EFI_ERROR(status);
  DEBUG ((EFI_D_INIT, "--ImxClkPwrInit S\r\n"));
}
...
VOID SdhcInit ()
{
  ...
  DEBUG ((EFI_D_INIT, "SdhcInit2 S GPIO4_IO30\r\n"));
  ImxGpioDirection(IMX_GPIO_BANK4, 30, IMX_GPIO_DIR_OUTPUT);
  DEBUG ((EFI_D_INIT, "SdhcInit2 S GPIO 30 DIR_OUTPUT\r\n"));
  ImxGpioWrite(IMX_GPIO_BANK4, 30, IMX_GPIO_LOW);
  DEBUG ((EFI_D_INIT, "SdhcInit2 S GPIO 30 LOW\r\n"));
  //
...
VOID EhciInit ()
{
    ...
    DEBUG ((EFI_D_INIT, "S EhciInit( )  Pin-muxed OTG Over Current\r\n"));
    ...
    DEBUG ((EFI_D_INIT, "S EhciInit( )  Pin-muxed and enabled OTG power\r\n"));
...
/**
  Initialize I2C modules on the iMX6 Solo SOM and perform required pin-muxing
**/
VOID I2cInit ()
{
...
/**
  Initialize PCI Express module on the iMX6 Solo SOC and perform required pin-muxing
**/
VOID PcieInit ()
{
    //
...
/**
    Initalize the iMX6 Solo SoM Uarts
**/
VOID UartInit()
{
    ...
    //
    // 2. Solo pin mux for UART 1 and UART 3
    //
...
{
    DEBUG ((DEBUG_INFO, "++S ArmPlatformInitialize(%Xh)\r\n", MpId));
  ...
  // TimerConstructor to initialize the timer.
    TimerConstructor ();
    SdhcInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) SDHC2 initialized.\r\n", MpId));
    EhciInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) USB initialized.\r\n", MpId));
    EnetInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) Enet initialized.\r\n", MpId));

    I2cInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) i2c initialized.\r\n", MpId));

    SpiInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) Spi initialized.\r\n", MpId));
    PcieInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) PCIe initialized.\r\n", MpId));

    SetupAudio ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) Audio initialized.\r\n", MpId));

    GpioInit ();
    DEBUG ((DEBUG_INIT, "ArmPlatformInitialize(S %Xh) GPIO initialized.\r\n", MpId));

    PwmInit ();
	
  Tpm2AcpiControlAreaInit ();
	
    DEBUG ((DEBUG_INIT, "--S ArmPlatformInitialize(%Xh)=ok\r\n", MpId));
