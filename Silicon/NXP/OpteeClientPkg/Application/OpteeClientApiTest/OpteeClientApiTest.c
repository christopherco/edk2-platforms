...
/**
 Test Case: TEEC_Storage
  - This uses the "Storage" test TA that has a read/write functions
**/
VOID
TeecStorageTest (
  )
{
  ...
  VERIFY_ARE_EQUAL (
    TEEC_Result,
    TEEC_SUCCESS,
    TEEC_InitializeContext (NULL, &mContext));

  C_ASSERT (sizeof (TeecUuid) == sizeof (gOpteeStorageTaGuid));
  CopyMem (&TeecUuid, &gOpteeStorageTaGuid, sizeof (TeecUuid));
  ...
  mSharedMem.size = EFI_PAGE_SIZE;
  mSharedMem.flags = 0;

  VERIFY_ARE_EQUAL (
    TEEC_Result,
    TEEC_SUCCESS,
    TEEC_AllocateSharedMemory (&mContext, &mSharedMem));
  VERIFY_IS_NOT_NULL (mSharedMem.buffer);

  // This command should write to the file.
  {
    TEEC_Operation TeecOperation = { 0 };
    CHAR16 Str[] = L"Hello World";
    TeecOperation.params[0].tmpref.buffer = mSharedMem.buffer;
    TeecOperation.params[0].tmpref.size = sizeof(Str);

    ZeroMem (mSharedMem.buffer, mSharedMem.size);
    CopyMem (mSharedMem.buffer, L"Hello World", TeecOperation.params[0].tmpref.size);
    TeecOperation.paramTypes = TEEC_PARAM_TYPES (
    ...
    LOG_COMMENT (
      "Writing Buffer %s (size=0x%p)\n",
      (CHAR16 *) mSharedMem.buffer,
      TeecOperation.params[0].tmpref.size);
    ...
    VERIFY_ARE_EQUAL (
      ...
      TEEC_InvokeCommand (
        &mSession,
        OPTEE_TEST_STORAGE_CMD_WRITE,
        &TeecOperation,
        &ErrorOrigin));
  }

  // This command should read from the file.
  {
    TEEC_Operation TeecOperation = { 0 };
    ...
    ZeroMem (mSharedMem.buffer, mSharedMem.size);
    TeecOperation.params[0].tmpref.buffer = mSharedMem.buffer;
    TeecOperation.params[0].tmpref.size = mSharedMem.size;
    ...
    VERIFY_ARE_EQUAL (
      ...
      TEEC_InvokeCommand (
        &mSession,
        OPTEE_TEST_STORAGE_CMD_READ,
        &TeecOperation,
        &ErrorOrigin));

    LOG_COMMENT (
      "Read Buffer %s (size=0x%p)\n",
      (CHAR16 *) mSharedMem.buffer,
      TeecOperation.params[0].tmpref.size);
  }
  ...
  {
    TEEC_Operation TeecOperation = { 0 };
    CHAR16 *Str = mSharedMem.buffer;
    UINTN StrLength = mSharedMem.size / sizeof (CHAR16);
    UINTN i;
  ...
  //
  {
    ...
    LOG_COMMENT (
      "Read Buffer (size=0x%p)\n",
      TeecOperation.params[0].tmpref.size);

    for (i = 0; i < StrLength; i++) {
      if (Str[i] != L'A') {
        VERIFY_IS_TRUE (FALSE, "Buffer mismatch at i=0x%p", i);
      }
    }
  }
}
