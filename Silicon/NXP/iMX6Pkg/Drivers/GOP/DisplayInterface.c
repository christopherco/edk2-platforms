    ...
    )
{
    ...
    // TODO Figure why set we cannot swap set index. Currently set 3 is used
    // for pixel clock and set 0 is blank signal. If this is swapped display
    // output would be wrong, why?
    {
    ...
    // (37.4.10.3 Timing generator) The timing generator is built of 10 counters
    // Need to get recommendation on how counter needs to be setup
    // Setup counter, the DI has 9 counter that utilize the timebase generator and waveform generator
    {
        // Internal HSYNC
        ConfigureSyncWave(
            ipuMmioBasePtr,
