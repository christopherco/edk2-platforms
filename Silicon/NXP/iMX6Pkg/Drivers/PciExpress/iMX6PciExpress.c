    ...
    )
{
    ...
    if (EFI_ERROR(status)) {
        PCIE_ERROR("Fail to disable PCIe Phy\n");
        goto Exit;
    }
    ...
    )
{
    ...
    //
    // Enable and ungate SATA clock
    //
    if (State == IMX_CLOCK_GATE_STATE_ON) {
        // Clear CLK1 IBEN
        MmioWrite32(
            (UINTN)&ccmAnalogRegisters->MISC1_CLR,
            0x01 << 12);

        // Select SATA ref clock
        MmioWrite32(
            (UINTN)&ccmAnalogRegisters->MISC1_CLR,
            0x1F << 0);

        MmioWrite32(
            (UINTN)&ccmAnalogRegisters->MISC1_SET,
            0x0B << 0);
    ...
    //
    // From PCIe perspective do not decide to turn off SATA clock let SATA
    // driver decide that instead. A better solution is create a centralize
    // location that keeps a refcount.
    //
#if !defined(CPU_IMX6SX)
    ImxClkPwrSetClockGate(IMX_SATA_CLK_ENABLE, IMX_CLOCK_GATE_STATE_ON);
#endif
...
//
// A very simple version of PCI enumerator that enumerates all available
// PCI device and does basic initialization. For future work add PCIBusDXE
// that would perform the necassary enumeration.
//
    ...
    )
{
    ...
    *AlignAddress = 0;
    ...
    // we would still alight to the upper boundary as bit [19:00] is assume to
    ...
    )
{
    ...
    if (!EFI_ERROR(status) && (PciDevice->Hdr).VendorId != 0xffff) {
        ...
        status = PCIePciRead(
            ...
            PciDevice);
        if (EFI_ERROR(status)) {
            PCIE_ERROR("Fail to read PCI config space\n");
        }
    } else {
    ...
    )
{
    ...
    for (barOffset = 0x10, barIndex = 0;
        ...
        status = PCIeGetMemoryBarResource(
            ...
            IsBridgeDevice);
        if (EFI_ERROR(status)) {
            PCIE_ERROR("Fail to acquire BAR resource\n");
            goto Exit;
        }
    ...
    )
{
    ...
    if (IS_PCI_BRIDGE (&pciDevice)) {
        ...
        //
        // No support for IO and prefetch memory, refer to InitializePpb
        //
        ...
        status = PCIeParseAssignBar(
            ...
            TRUE);
        if (EFI_ERROR(status)) {
            PCIE_ERROR("Fail to assign resource to PCI bridge\n");
            goto Exit;
        }
        ...
        status = PCIeParseAssignBar(
            ...
            FALSE);
        if (EFI_ERROR(status)) {
            PCIE_ERROR("Fail to assign resource to PCI device\n");
            goto Exit;
        }
    }
    ...
    )
{
    ...
    //
    // A very simple version of PCI enumerator that enumerates all available
    // PCI device and does basic initialization. For future work remove this
    // and add PCIBusDXE support.
    //
    for (devNumber = 0; devNumber <= PCI_MAX_DEVICE; ++devNumber) {
        for (functionNumber = 0; functionNumber <= PCI_MAX_FUNC; ++functionNumber) {
            ...
            if (EFI_ERROR(status)) {
                PCIE_ERROR(
                    "Fail to configure device B:%02d D:%02d F:%02d\n",
                    BusNumber,
                    devNumber,
                    functionNumber);
                continue;
            ...
            if (IS_PCI_BRIDGE (&pciDevice)) {
                ...
                status = PCIePciWrite(
                    ...
                    &busRegister);
                if (EFI_ERROR(status)) {
                    PCIE_ERROR("Fail to update bridge bus number %d\n", BusNumber);
                    continue;
                }
                ...
                status = PCIeSimpleScanBusAndAssignResource(
                    BusNumber + 1);
                if (EFI_ERROR(status)) {
                    PCIE_ERROR("Fail to scan new bus %d\n", BusNumber + 1);
                    continue;
                }
                ...
                if (EFI_ERROR(status)) {
                    PCIE_ERROR(
                        "Fail to update subordinate bus number %d\n",
                        BusNumber);
                    continue;
                }
                ...
                status = PCIeGetMemoryBarResource(
                    ...
                    FALSE);
                if (EFI_ERROR(status)) {
                    PCIE_ERROR("Fail to realign resource\n");
                    goto Exit;
                }
            } 
    ...
    )
{
    ...
    if (EFI_ERROR(status)) {
        ...
        PCIE_ERROR("Fail to enable PCIe gates\n");
        goto Exit;
    }
    ...
    if (EFI_ERROR(status)) {
        PCIE_ERROR("Fail to enable PCIe Phy\n");
        goto Exit;
    }
    ...
    if (EFI_ERROR(status)) {
        PCIE_ERROR("Fail to setup PCI bridge\n");
        goto Exit;
    }

    status = PCIeSetLinkStatus(TRUE);
    if (EFI_ERROR(status)) {
        PCIE_ERROR("Fail to enable PCIe link\n");
        goto Exit;
    }
...
Exit:
    ...
    if (EFI_ERROR(status)) {
        PCIE_ERROR("Fail to initialize PCIe, disabling controller\n");
